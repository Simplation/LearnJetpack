...
public class Drawables {
    ...
    public static void setViewBackground(
        ...
        if (count < 1) {
            ...
            if (isDefaultNull || count == 1) {
                // 当设置了 margin（非 view 的 margin）时，InsetDrawable 会导致 view 本身的 padding 失效
    ...
    public static Drawable create(
        ...
        if (shapeMode == INVALID && solidColor == null && strokeColor == INVALID
            ...
            // 这里需要判断 empty，因为有可能只设置了一个 state 的 drawable，那么其他 state 的就是 empty 了
            return null;
        }
        ...
        if (shapeMode == ShapeMode.RING) {
            // 由于 GradientDrawable 中没有 ring 相关的公开 API，所以使用反射，若对性能有要求，请注意。
    ...
    public @interface ShapeMode {
         ...
         * 画线时，有几点特性必须要知道的：
         * 1. 只能画水平线，画不了竖线；
         * 2. 线的高度是通过 stroke 的 android:width 属性设置的；
         * 3. size 的 android:height 属性定义的是整个形状区域的高度；
         * 4. size 的 height 必须大于 stroke 的 width，否则，线无法显示；
         * 5. 线在整个形状区域中是居中显示的；
         * 6. 线左右两边会留有空白间距，线越粗，空白越大；
         * 7. 引用虚线的 view 需要添加属性 android:layerType，值设为 "software"，否则显示不了虚线。
         */
