...
object ActivityMessenger {
     ...
     *  </pre>
     *
     * @param TARGET 要启动的 Activity
     * @param starter 发起的 Activity
     * @param params extras 键值对
     */
    ...
    /**
     *  Fragment 跳转，同 [Activity.startActivity]
     *  示例：
     *  <pre>
     *      // 不携带参数
     *      ActivityMessenger.startActivity<TestActivity>(this)
    ...
    /**
     * Adapter 跳转，同 [Context.startActivity]
     *  示例：
     *  <pre>
     *      // 不携带参数
    ...
    /**
     *  Adapter 里面跳转，同 [Context.startActivity]
     *  示例：
     *  <pre>
     *      // 不携带参数
     ...
     * @param starter 发起的 Activity
     * @param params extras 键值对
     * @param callback onActivityResult 的回调
     */
    ...
    /**
     *  Fragment 跳转，同 [Activity.startActivityForResult]
     *  示例：
     *  <pre>
     *      // 不携带参数
     *      ActivityMessenger.startActivityForResult<TestActivity>(this) {
    ...
    /**
     *  Fragment 调用，作用同 [Activity.finish]
     *  示例：
     *  <pre>
     *      ActivityMessenger.finish(this, "Key" to "Value")
     *  </pre>
     *
     * @param src 发起的 Fragment
     * @param params extras 键值对
     */
 ...
 * 如 getStringExtra()、getIntExtra()、getSerializableExtra() 等方法通通不用
...
fun <O> Intent.get(key: String, defaultValue: O? = null): O? {
    try {
        ...
        val map = IntentFieldMethod.mMap.get(extras) as? Map<*, *> ?: return defaultValue
...
fun <O> Bundle.get(key: String, defaultValue: O? = null): O? {
    try {
        ...
        val map = IntentFieldMethod.mMap.get(this) as? Map<*, *> ?: return defaultValue
        return map[key] as? O ?: return defaultValue
    } catch (e: Exception) {
        // Ignore
    }
...
class ActivityExtras<T>(private val extraName: String, private val defaultValue: T) :
    ...
    override fun getValue(thisRef: Activity, property: KProperty<*>): T {
        // 如果 extra 不为空则返回 extra
        // 如果 extra 是空的，则判断 intent 的参数的值，如果值不为空，则将值赋予 extra，并且返回
        // 如果 intent 参数的值也为空，则返回 defaultValue，并且将值赋予 extra
...
/**
 * 以下方法只是把 ActivityMessenger 里面的方法变成了扩展方法
 */
...
fun FragmentActivity.startActivity(target: KClass<out Activity>, vararg params: Pair<String, Any>) =
    startActivity(Intent(this, target.java).putExtras(*params))

fun Fragment.startActivity(target: KClass<out Activity>, vararg params: Pair<String, Any>) =
    activity?.run {
        startActivity(Intent(this, target.java).putExtras(*params))
    }
